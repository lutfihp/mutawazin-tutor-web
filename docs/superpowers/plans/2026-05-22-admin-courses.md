...
**Goal:** Build `/admin/courses` — a new admin page for full course CRUD (list, create, edit, delete) using three new backend endpoints plus the existing course list endpoint.

**Architecture:** Follows the established admin page pattern: a pass-through `+layout.svelte`, an admin-only `+page.server.ts`, and a CSR `+page.svelte` that loads all data in `onMount`. Three inline modals (Create, Edit, Delete) use the shared `<Modal>` and `<Button>` components. The `<DropdownMenu>` component handles per-row actions.

**Tech Stack:** SvelteKit (Svelte 5 runes), Tailwind CSS v3, existing `api` client, `<Modal>`, `<Button>`, `<Badge>`, `<Card>`, `<DropdownMenu>`.

---

### Task 1: Add Courses to admin sidebar

**Files:**
- Modify: `src/lib/components/layout/Sidebar.svelte:31`

- [ ] **Step 1: Add the Courses item to the admin array**

  In `src/lib/components/layout/Sidebar.svelte`, find the admin items array (around line 31). After the subjects entry, add:
  ...
  `nav.courses` = "Courses" (EN) / "Kursus" (ID) — already in both locale files. `BookOpen` is already imported.
...
- [ ] **Step 3: Commit**
  ...
  git commit -m "feat: add Courses link to admin sidebar"
  ```

---

### Task 2: Create route scaffolding

**Files:**
- Create: `src/routes/admin/courses/+layout.svelte`
- Create: `src/routes/admin/courses/+page.server.ts`

- [ ] **Step 1: Create `+layout.svelte`**

  Create `src/routes/admin/courses/+layout.svelte` with this content (pass-through only — parent `/admin/+layout.svelte` already provides `<AuthLayout>`):
...
- [ ] **Step 4: Commit**
  ...
  ```bash
  git add src/routes/admin/courses/+layout.svelte src/routes/admin/courses/+page.server.ts
  git commit -m "feat: scaffold admin courses route"
  ```

---

### Task 3: Build the complete courses page

**Files:**
- Create: `src/routes/admin/courses/+page.svelte`

- [ ] **Step 1: Create `+page.svelte` with the full implementation**

  Create `src/routes/admin/courses/+page.svelte` with this content:
...
- [ ] **Step 3: Commit**
  ...
  git commit -m "feat: build admin courses management page with create/edit/delete"
  ```
...
- [ ] **Step 2: Verify sidebar**
  ...
  Log in as `admin@mutawazin.com / changeme123`. Check the admin sidebar — "Courses" item should appear below "Subjects".

- [ ] **Step 3: Verify course list**

  Navigate to `http://localhost:5173/admin/courses`. The table should load with all courses. Columns: Course, Teacher (name from lookup), Age Categories (violet badges), Status, Enrolled count, Actions (`⋮`).

- [ ] **Step 4: Verify create**

  Click "Create Course". Modal opens. Select a teacher, select a subject, check one or more age categories (price fields appear inline), optionally fill description. Submit → course appears at top of list, modal closes.

- [ ] **Step 5: Verify edit**

  Click `⋮` → Edit on any course. Modal opens pre-filled. Change the teacher — yellow warning appears. Change status between draft/active/archived. Save → row updates in-place.

  If a course has enrolled students: open its edit modal → subject dropdown is disabled with note below it.

- [ ] **Step 6: Verify delete**

  Click `⋮` → Delete on any course. Confirmation modal opens with course name in title. Cancel → modal closes, row unchanged. Confirm → row removed from list.

  If a course has enrolled students: confirm delete → inline error "Unenroll all students before deleting this course" appears, modal stays open.
