# Admin Calendar Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a new `/admin/calendar` page that shows sessions across all teachers via `GET /calendar/admin`, supports teacher-filter, session create (`POST /sessions` with `teacher_id`), session edit (`PUT /sessions/:id`), and a recurring templates panel (shown only when filtered to a teacher).

**Architecture:** New route under `src/routes/admin/calendar/` following the existing admin sub-route pattern (pass-through layout, admin auth guard, CSR data loading in `onMount`). The page is a full adaptation of the teacher calendar page — same month grid, pill colours, recurring panel and modals — minus the availability panel, plus a teacher picker and a session-edit modal.

**Tech Stack:** Svelte 5 runes, Tailwind v3, `api` helper, `calendarGrid` / `toISODate` / `formatMonth` from `$lib/utils/date`, existing `Badge`, `Button`, `Modal`, `Card` UI components, svelte-i18n.
...
| Action | Path | Purpose |
|---|---|---|
| Modify | `src/locales/en.json` | Add `admin.filterByTeacher` key |
| Modify | `src/locales/id.json` | Add `admin.filterByTeacher` key |
| Modify | `src/lib/components/layout/Sidebar.svelte` | Add Calendar to admin nav |
| Create | `src/routes/admin/calendar/+layout.svelte` | Pass-through layout |
| Create | `src/routes/admin/calendar/+page.server.ts` | Admin auth guard |
| Create | `src/routes/admin/calendar/+page.svelte` | Full calendar page |

---

### Task 1: i18n key + Sidebar nav item
...
- [ ] **Step 1: Add `admin.filterByTeacher` to en.json**

  In `src/locales/en.json`, inside the `"admin"` object (after the last existing key), add:

  ```json
  "filterByTeacher": "Filter by teacher…"
  ```

- [ ] **Step 2: Add `admin.filterByTeacher` to id.json**

  In `src/locales/id.json`, inside the `"admin"` object at the same position, add:

  ```json
  "filterByTeacher": "Filter berdasarkan guru…"
  ```

- [ ] **Step 3: Add Calendar to admin sidebar**

  In `src/lib/components/layout/Sidebar.svelte`, find the `admin` array in `const items`. It currently ends with the `courses` item. Add a `calendar` item after it:
  ...
  `Calendar` is already imported from `lucide-svelte` (used by teacher/student nav). No import change needed.

- [ ] **Step 4: Commit**

  ```bash
  git add src/locales/en.json src/locales/id.json src/lib/components/layout/Sidebar.svelte
  git commit -m "feat: add admin calendar sidebar link and i18n key"
  ```
...
**Files:**
- Create: `src/routes/admin/calendar/+layout.svelte`
- Create: `src/routes/admin/calendar/+page.server.ts`

- [ ] **Step 1: Create the pass-through layout**
  ...
  This follows the same pattern as `src/routes/admin/courses/+layout.svelte`. The parent `/admin/+layout.svelte` already wraps everything in `<AuthLayout role="admin">`.
...
- [ ] **Step 3: Commit**
  ...
  ```bash
  git add src/routes/admin/calendar/+layout.svelte src/routes/admin/calendar/+page.server.ts
  git commit -m "feat: scaffold admin calendar route with auth guard"
  ```

---

### Task 3: Full admin calendar page

**Files:**
- Create: `src/routes/admin/calendar/+page.svelte`

The page adapts the teacher calendar (`src/routes/calendar/+page.svelte`) for admin use:
- Fetches from `GET /calendar/admin` instead of `GET /calendar/me`
- Adds teacher picker (`GET /admin/teachers`)
- No availability panel
- Session pills show teacher name prefix in unfiltered mode
- Clicking a pill opens a session-**edit** modal (admin can edit all fields via `PUT /sessions/:id`)
- Add session modal includes a required teacher picker, pre-filled when filter is active
- Recurring panel shown only when teacher filter is active

Notes on data:
- Teacher IDs from `GET /admin/teachers`: use `teacher.user_id ?? teacher.id` (same as `src/routes/admin/courses/+page.svelte`)
- Courses list: `GET /courses` (same endpoint as admin courses page — returns all courses for admin auth)
- Students list: `GET /admin/students` (returns array with `user_id` and `full_name`)

- [ ] **Step 1: Create `src/routes/admin/calendar/+page.svelte`**
  ...
  <script lang="ts">
  	...
  	// Courses filtered by selected teacher for create/recurring modals
  	...
  	function pillLabel(session: any): string {
  		...
  		if (!filteredTeacherId) {
  			...
  			return `${rec}${firstName ? firstName + ' · ' : ''}${time} ${session.title}`;
  		}
  		return `${rec}${time} ${session.title}`;
  	}
  	...
  	async function saveEdit() {
  		...
  		try {
  			...
  			await api.put(`/sessions/${selectedSession.id}`, {
  				...
  				student_id: eStudentId || undefined,
  	...
  	function openAddSession() {
  		...
  		sMode = 'online'; sPrice = '';
  		sTeacherId = filteredTeacherId; // pre-fill from active filter
  		addOpen = true;
  	}
  	...
  	<div class="grid calendar-panel:grid-cols-[1fr_280px] gap-5">
  							...
  							<div class="flex flex-col gap-0.5">
  								{#each daySessions.slice(0, 2) as session}
  									<button
  										...
  										title="{teacherMap[session.teacher_id] ?? ''}: {session.title}"
  									>
  										{pillLabel(session)}
  									</button>
  								{/each}
  		...
  		<div class="flex flex-col gap-4">
  			...
  			<div class="bg-white border border-border rounded-DEFAULT p-4">
  				...
  				{#if !filteredTeacherId}
  					<p class="text-sm text-text2">{$t('calendar.noRecurring')} Select a teacher to view their recurring sessions.</p>
  				{:else}
  					<div class="mb-3 p-2.5 bg-warningBg border border-warning/40 rounded-sm text-xs text-warningText leading-relaxed">
  						Recurring management for other teachers requires a backend update (recurring endpoints don't yet accept teacher_id). Create/edit will not work until then.
  					</div>
  ...
  {#if selectedSession}
  	<Modal open={editOpen} title="Edit Session" onclose={() => { editOpen = false; cancelConfirming = false; }}>
  		<div class="flex flex-col gap-4">
  			<!-- Title -->
  			<div class="flex flex-col gap-1.5">
  				<label for="eTitle" class="text-[13px] font-medium">{$t('calendar.modal.titleLabel')}</label>
  				<input id="eTitle" type="text" bind:value={eTitle}
  			...
  			{:else if selectedSession.type === 'private'}
  				<div class="flex flex-col gap-1.5">
  					<label for="eStudentId" class="text-[13px] font-medium">{$t('calendar.modal.studentLabel')}</label>
  					<select id="eStudentId" bind:value={eStudentId}
  						class="w-full bg-white border border-border rounded-sm px-3 py-2.5 text-sm focus:outline-none focus:border-primary">
  	...
  	<form bind:this={rFormEl} onsubmit={handleRecurringSubmit} class="flex flex-col gap-4">
  		...
  		{:else}
  			<div class="flex flex-col gap-1.5">
  				...
  				<input id="rStudentId" type="text" bind:value={rStudentId} required placeholder="Student ID"
  					class="w-full bg-white border border-border rounded-sm px-3 py-2.5 text-sm focus:outline-none focus:border-primary" />
  			</div>
  		{/if}
  		<div class="flex flex-col gap-1.5">
...
- [ ] **Step 2: Commit**
  ...
  git commit -m "feat: add admin calendar page with teacher filter, session edit, and recurring panel"
  ```
...
- [ ] **Step 1: Run type check**
  ...
  - `'$t(...)' is not assignable to 'string[]'` on `calendar.modal.days` — the cast `as unknown as string[]` is already in the code; if the error appears elsewhere, apply the same cast.
  - `Property 'user_id' does not exist` on student objects — if `/admin/students` returns `id` instead of `user_id`, change `student.user_id` to `student.id` in both the edit and add session modals.

- [ ] **Step 2: Commit**

  ```bash
  git add -A
  git commit -m "chore: verify admin calendar type-check passes"
  ```

  If no changes were needed in Step 1, skip this commit.
